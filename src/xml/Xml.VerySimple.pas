...
uses
  Classes, Generics.Defaults, Generics.Collections, dialogs;
...
type
  ...
  public
    ...
    function FindEx(Attribute, Value: String): TXmlNodeList;
    function FindEx2(Attribute, Value: String): TXmlNode;
    function FindExPos(Attribute, Value: String): TXmlNode;
...
function TXmlNode.FindEx2(Attribute, Value: String): TXmlNode;
var
  Node: TXmlNode;
begin
  Result := NIL;
  for Node in ChildNodes do
  begin
    if (Node.HasAttribute(Attribute)) and
      (Node.Attribute[Attribute] = Value) then
    begin
      Result := Node;
      Break;
...
function TXmlNode.FindExPos(Attribute, Value: String): TXmlNode;
var
  Node: TXmlNode;
begin
  Result := NIL;
  ...
  begin
    if (Node.HasAttribute(Attribute)) and
      (pos(Value, Node.Attribute[Attribute]) > 0) then
    begin
      Result := Node;
      Break;
    end;
...
function TXmlNode.FindEx(Attribute, Value: String): TXmlNodeList;
var
  Node: TXmlNode;
begin
  Result := TXmlNodeList.Create(False);
  for Node in ChildNodes do
    if (Node.HasAttribute(Attribute)) and (Node.Attribute[Attribute] = Value) then
    begin
      Result.Add(Node);
    end;
end;
...
{ if we want to get each node of a list we do this:
var
  node: TXMLnode;
  nodelist: TXMLNodelist;
begin
nodelist:= XmlItemList.Root.FindEx('category','food');
for Node in nodelist do
  begin
      showmessage(Node.Attribute['name'])
end;
}
