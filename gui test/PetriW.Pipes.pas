...
type
  TPBPipeServerReceivedDataEvent = procedure(AData: string; RawData: TBytes; len: integer) of object;
  ...
  public
    ...
    function SendData(AData: string): boolean; overload;
    function SendDataEx(var AData: TBytes; index,len: integer): boolean;
...
function TPBPipeClient.SendDataEx(var AData: TBytes; index,len: integer): boolean;
var
  bytesRead: Cardinal;
  success: boolean;
begin
  result := CallNamedPipe(PWideChar(FPath), @AData[index], len, @success, SizeOf(Integer), bytesRead, NMPWAIT_USE_DEFAULT_WAIT);
end;

function TPBPipeClient.SendData(AData: string): boolean;
var
  bytesRead: Cardinal;
  success: boolean;
begin
  result := CallNamedPipe(PWideChar(FPath), PWideChar(AData), length(AData) * SizeOf(Char), @success, SizeOf(Integer), bytesRead, NMPWAIT_USE_DEFAULT_WAIT);
  // aqui deberiamos poner <<result := 'test'>> asi conseguimos que no se ejecuten los pipes!
end;
